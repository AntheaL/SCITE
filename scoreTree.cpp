...
double* getAttachmentScoresFast(int*parent, int n, double*** logScores, int* dataVector, int*bft){
	...
	for(int i=1; i<=n; i++){                                                              // try all attachment points (nodes in the mutation tree)
		...
		attachmentScore[node] -= logScores[node][dataVector[node]][0];
		attachmentScore[node] += logScores[node][dataVector[node]][1];
	}
...
double rootAttachementScore(int n, double*** logScores, int* dataVector){
	...
	for(int gene=0; gene<n; gene++){                          // sum over log scores for all other nodes in tree
		score += logScores[gene][dataVector[gene]][0];      // none of them is ancestor of the sample as it is attached to root
	}
	return score;
}
...
double* getTrueScores(int*** matrix, int n, double*** logScores){
	...
	for(int node=0; node<=n; node++){
		scores[node] = getTrueScore(matrix[node], logScores[node]);
	}
	return scores;
}
...
double*** getLogScores(double* FD, double* AD1, double* AD2, double* CC, int n){
    double*** logScores = init_double3DMatrix(n, 4, 2, 0.0);
	for(int i=0; i<=n; i++){
        logScores[i][0][0] = log(1.0-CC[i]-FD[i]);  // observed 0, true 0
        logScores[i][1][0] = log(FD[i]);         // observed 1, true 0
        if(CC[i]!=0.0){
            logScores[i][2][0] = log(CC[i]);         // observed 2, true 0
        }
        else{                                //  to capture case where CC=0, because log(0) = -infinity
            logScores[i][2][0] = 0.0;           // CC=0 should only occur when there are no 2's (double mutations) in the matrix
        }
        logScores[i][3][0] = log(1.0);          // value N/A,  true 0
        logScores[i][0][1] = log(AD1[i]);      // observed 0, true 1
        logScores[i][1][1] = log(1.0-(AD1[i]+AD2[i]));     // observed 1, true 1
        if(AD2[i] != 0.0){
            logScores[i][2][1] = log(AD2[i]);     // observed 2, true 1
        }
        else{
            logScores[i][2][1] = 0;
        }
        logScores[i][3][1] = log(1.0);          // value N/A,  true 1
    }
    return logScores;
}
...
double*** getScores(double* FD, double* AD1, double* AD2, double* CC, int n){

  double*** scores = init_double3DMatrix(n, 4, 2, 0.0);
    for(int i=0; i<=n; i++){
        scores[i][0][0] = 1.0-CC[i]-FD[i];  // observed 0, true 0
        scores[i][1][0] = FD[i];         // observed 1, true 0
        scores[i][2][0] = CC[i];         // observed 2, true 0
        scores[i][3][0] = 1.0;          // value N/A,  true 0
        scores[i][0][1] = AD1[i];          // observed 0, true 1
        scores[i][1][1] = 1.0-(AD1[i]+AD2[i]);     // observed 1, true 1
        scores[i][2][1] = AD2[i];               // observed 2, true 1
        scores[i][3][1] = 1.0;               // value N/A,  true 1
    }
    return scores;
}
